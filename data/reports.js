 ...
 * Callers decide how to interpret it; update methods attempt reportId first, then _id fallback.
 */
 ...
 * - Some versions return a ModifyResult-like wrapper containing { value: <doc>, ... }
 * - Others may return the document directly depending on driver/version/options
 */
function unwrapFindOneAndUpdateResult(res) {
  if (!res) return null;

  // If a wrapper is returned, prefer res.value
  if (
    typeof res === 'object' &&
    res !== null &&
  ...
  // Otherwise assume res is already the document
  return res;
}
...
/**
 * Create a new report for a given target.
 * Initializes vote aggregates and duplicate-flag aggregates on the report document.
 */
export async function createReport({ targetType, targetId, text, createdBy }) {
  ...
  for (let i = 0; i < 3; i++) {
    const doc = {
      ...
      // Aggregate totals for duplicate-flagging (kept in sync by /api/reports/:id/duplicate endpoints)
      duplicateFlags: {
        flagCount: 0,
        weightTotal: 0,
        topCandidateReportId: '',
        topCandidateCount: 0,
        topCandidateWeight: 0
      },

      // Aggregate totals for voting (kept in sync by /api/reports/:id/vote endpoints)
      votes: {
        upVote: 0,
        downVote: 0,
        voteCount: 0,
...
/**
 * Fetch a report by custom reportId; also supports MongoDB _id as a fallback for robustness.
 */
...
/**
 * Get recent reports created by a specific user.
 */
...
/**
 * Get recent reports for a given target (station/elevator/aps/ramp).
 */
...
/**
 * Update stored vote aggregates on a report (stores both weighted and raw totals).
...
export async function updateReportVotes(reportIdOrMongoId, total = {}) {
  ...
  // Note: totals come from data/votes.js as upVotes/downVotes (plural).
  ...
  // Note: weighted totals come from data/votes.js as uWeight/dWeight.

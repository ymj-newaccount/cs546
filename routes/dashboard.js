...
router.get('/', ensureLoggedInPage, async (req, res, next) => {
  try {
    ...
    // ---------------------------
    const or = [];
    if (stationIds.length) or.push({ targetType: 'station', targetId: { $in: stationIds } });
    if (apsIds.length) or.push({ targetType: 'aps', targetId: { $in: apsIds } });
    if (rampIds.length) or.push({ targetType: 'ramp', targetId: { $in: rampIds } });

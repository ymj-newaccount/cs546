...
function normalizeKind(raw) {
  ...
  if (!k) throw httpError('kind is required', 400);
  if (!['aps', 'ramp'].includes(k)) throw httpError('kind must be aps or ramp', 400);
  return k;
}
...
function normalizeId(raw) {
  ...
  if (!id) throw httpError('id is required', 400);
  // Defensive limit against abuse / extremely long path segments
  if (id.length > 200) throw httpError('id is too long', 400);
  return id;
}
...
async function handleFollowToggle(req, res, next, shouldFollow) {
  try {
    ...
    if (!ok) return res.status(404).json({ error: 'Crossing not found.' });

    const userId = req.session.user._id;

    const updatedUser = shouldFollow
      ? await addCrossingBookmark(userId, kind, id)
      : await removeCrossingBookmark(userId, kind, id);

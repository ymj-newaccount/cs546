...
(function() {
  ...
  // Get station ID from URL path (e.g., /station/123)
  const getStationId = () => {
  ...
  // ========== ADA Summary Section ==========
  const initAdaSummary = () => {
    const adaSection = document.querySelector('.ada-summary');
    if (!adaSection) return;

    // You could add interactive features here, like:
    // - Expand/collapse details
    // - Show/hide accessibility features list
    // - Link to accessibility guide
  };

  // ========== Outage History Section ==========
  const initOutageHistory = () => {
    const outageSection = document.querySelector('.outage-summary');
    if (!outageSection) return;

    const outageList = outageSection.querySelector('ul');
    
    // If no outages exist, show a message
    if (!outageList || outageList.children.length === 0) {
      const noOutagesMsg = document.createElement('p');
      noOutagesMsg.textContent = 'No previous outages reported for this station.';
      noOutagesMsg.classList.add('no-outages');
      outageSection.appendChild(noOutagesMsg);
    }
  };
  ...
  const handleReportSubmit = async (e) => {
    ...
    try {
      ...
      formData.append('targetType', 'station');
      formData.append('targetId', stationId);
      ...
      // Add new report to the list (prepend to top)
  ...
  const init = () => {
    ...
    console.log('Station page initialized for station:', stationId);
  };
